...
module.exports = function ($scope, $timeout, $filter, AdminAccessService, uiError, ADMIN_ACCESS_CONSTANT, accessLevelProvider) {
    ...
    var _clearInvalidAccessLevels = function () {
        accessLevelProvider.clearInvalidAccessLevels($scope.securityPolicies);
        if ($scope.newSecurityPolicy) {
            accessLevelProvider.clearInvalidAccessLevels([$scope.newSecurityPolicy]);
        }
    };
                ...
                .then(function () {
                    if (isNew && !cachedNewRow) {
                        ...
                        if (isDefault) {
                            ...
                            accessLevelProvider.assertLevelValidity($scope.accessLevelItems, currentSecurityPolicy);
                        }
                        $scope.displayAddMode = false;
                    }
                })
